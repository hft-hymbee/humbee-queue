...
def send_sms_notification(
    ...
    logger.info("SMS task started", extra={**log_extra, "event": "task_started"})
    ...
    if record is None:
        logger.error(
            "SMS task aborted: NotificationHistory record not found after polling",
    ...
    if record.status == NotificationStatus.SENT.value:
        logger.info(
            "SMS task: already sent, skipping",
    ...
    logger.info("SMS task: marked PROCESSING", extra={**log_extra, "event": "task_processing"})
    ...
    try:
        ...
        logger.error(
            f"SMS task failed: validation error — {val_err}",
        ...
        logger.warning(
            f"SMS task failed (retryable): {exc}. Celery will autoretry. "
        ...
        logger.error(
            f"SMS task failed (unexpected): {exc}",
            extra={**log_extra, "event": "task_failed", "status": "FAILED"},
        )
        raise exc
